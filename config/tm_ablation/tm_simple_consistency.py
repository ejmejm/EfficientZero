...
class TargetMatcherConfig(BaseConfig):
    ...
    def new_game(self, seed=None, save_video=False, save_path=None, video_callable=None, uid=None, test=False, final_test=False):
        if test:
            env = TargetMatcher(n_targets=4, n_distractors=2, n_buttons=3, max_timesteps=self.test_max_moves)
        else:
            env = TargetMatcher(n_targets=4, n_distractors=2, n_buttons=3, max_timesteps=self.max_moves)
        # if seed is not None:
        #     env.seed(seed)
